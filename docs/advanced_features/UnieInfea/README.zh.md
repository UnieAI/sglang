...
- **2026-01-18**：UnieInfra 動態策略控制器（DSC）完整實現

  **核心功能**：
  - ✅ **NGRAM 動態路由**：在每個 decode step 智慧決策是否使用 NGRAM 推測解碼
  - ✅ **多重 Gate 機制**：batch size、序列長度、輸出長度、accept-rate EMA 四種 gate 可組合使用
  - ✅ **Accept-Rate 自適應**：使用 EMA + 遲滯機制自動適應工作負載品質
  - ✅ **探測機制**：gate 關閉時定期探測，檢測條件改善並自動重新啟用
  - ✅ **零開銷回退**：bypass 時使用原生 decode 路徑，無額外開銷
  - ✅ **PD 分離兼容**：與 Prefill/Decode disaggregation 完全兼容
  - ✅ **LMCache 整合**：可與 LMCache 分層快取正交使用

  **解決的問題**：
  - 🎯 高併發時 NGRAM 效能下降 → batch size gate 自動切換
  - 🎯 低接受率導致減速 → accept-rate EMA gate 動態監控
  - 🎯 長序列記憶體壓力 → seq len gate 限制使用場景
  - 🎯 長輸出命中率低 → output len gate 精準控制
  - 🎯 工作負載品質可變 → 自適應機制自動調整

  **預期提升**：
  - 📈 保留低 batch size 的 NGRAM 加速（2-3x token generation）
  - 📈 避免高併發時的吞吐下降（維持標準 decode 效能）
  - 📈 無需重啟即可動態切換（毫秒級決策延遲）
  - 📈 適應各種工作負載（RAG、程式碼生成、對話等）
